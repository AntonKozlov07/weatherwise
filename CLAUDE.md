...
44. **The One Call version resolves at runtime, and setting the key is the only configuration step.** Versions are tried in order (4.0, then 3.0) and the winner is cached per server instance, so the fallback costs one extra request once. This exists because the reachable version cannot be determined without calling it: OWM answers 401 at the gateway for every version, including ones that do not exist. Deployment is therefore just the Vercel environment variable, with no code edit if the account lands on a different version.
