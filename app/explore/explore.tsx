...
type AccuracyState = { summary: string[] };

export function Explore() {
  const preferences = usePreferences();
  const [world, setWorld] = useState<WorldSnapshot[]>([]);
  const [history, setHistory] = useState<HistoryState | null>(null);
  const [accuracy, setAccuracy] = useState<AccuracyState | null>(null);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`/api/history?lat=${coordinates.latitude}&lon=${coordinates.longitude}`)
        .then((response) => (response.ok ? response.json() : null))
        .catch(() => null),
      fetch(`/api/accuracy?lat=${coordinates.latitude}&lon=${coordinates.longitude}`)
        .then((response) => (response.ok ? response.json() : null))
        .catch(() => null),
    ]).then(([worldPayload, historyPayload, accuracyPayload]) => {
      if (cancelled) return;

      setAccuracy(
        accuracyPayload?.summary?.length
          ? { summary: accuracyPayload.summary }
          : null,
      );
  ...
  return (
    <div className="screen relative">
      <header className="page-gutter flex items-center justify-between pt-2">
        <h1 className="screen-title">Explore</h1>

        {/* The standings live behind an icon rather than as a third section.
            They answer a question you go looking for, and a board of fifty at
            the foot of the screen buried the two things that answer questions
            you already had (Decisions Log 114). */}
        <LeaderboardButton cities={world} units={preferences.units} />
      </header>
      ...
      <div className="screen-scroll page-gutter flex flex-col gap-8 pb-6 pt-5">
        ...
        >
          ...
          {!loading && world.length > 0 && (
            <div className="grid grid-cols-2 gap-3">
              ...
              {world
                .filter((city) => city.id !== HOME_CITY_ID && city.featured)
                .map((city) => (
                  <WorldCard
                    key={city.id}
                    city={city}
        ...
        {accuracy && (
          ...
          >
            <h2 className="type-label text-2xs">How good is the forecast here</h2>

            <ul className="flex flex-col gap-2">
              {accuracy.summary.map((line) => (
                <li
                  key={line}
                  className="rounded-card bg-surface px-4 py-3 text-sm leading-relaxed text-text-dim"
                >
            ...
            {/* Stated, never applied. Quietly shifting the displayed
                temperature would make this app disagree with every other
                forecast without saying why (Decisions Log 115). */}
            <p className="type-label text-2xs text-text-faint">
